module Hydra
class ModsBook < ActiveFedora::NokogiriDatastream       
  ...
  set_terminology do |t|
    ...
    t.book(:path=>'relatedItem', :attributes=>{:type=>"host"}) {
    ...
    def insert_contributor(type, opts={})
      case type.to_sym 
      when :person
        node = Hydra::ModsBook.person_template
        nodeset = self.find_by_terms(:person)
      when :organization
        node = Hydra::ModsBook.organization_template
        nodeset = self.find_by_terms(:organization)
      when :conference
        node = Hydra::ModsBook.conference_template
        nodeset = self.find_by_terms(:conference)
      else
        ActiveFedora.logger.warn("#{type} is not a valid argument for Hydra::ModsBook.insert_contributor")
        node = nil
        index = nil
      end
    ...
    def self.dc_relator_terms
      ...
      def to_solr(solr_doc=Solr::Document.new)
        super(solr_doc)
        extract_person_full_names_and_computing_ids.each {|pfn| solr_doc << pfn }
        extract_person_organizations.each {|org| solr_doc << org }
        solr_doc << {:object_type_facet => "Book"}
        solr_doc
      end
